-- depends_on: {{ ref('audit_int_nba__game_betting_results') }}

{{
    config(
        materialized='incremental',
        unique_key=['game_date', 'game_id', 'market_key', 'side'],
        incremental_strategy='merge'
    )
}}

-- Joins completed games with consensus lines to determine cover/push/miss
-- outcomes for every bet type. Core "did the bet win?" model.
-- Grain: one row per (game_date, game_id, market_key, side).
-- Links Odds API → BDL via (game_date, home_team) natural key.
-- WAP: incremental runs publish from audit table; full refresh rebuilds from source.

{% if is_incremental() %}

select * from {{ ref('audit_int_nba__game_betting_results') }}
