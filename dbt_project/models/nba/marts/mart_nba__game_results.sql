-- depends_on: {{ ref('audit_mart_nba__game_results') }}
...
-- Central fact table: one row per completed game with scores and
-- all betting outcomes (spread, moneyline, total).
-- Grain: one row per (game_date, game_id).
-- WAP: incremental runs publish from audit table; full refresh rebuilds from source.

{% if is_incremental() %}

select * from {{ ref('audit_mart_nba__game_results') }}
